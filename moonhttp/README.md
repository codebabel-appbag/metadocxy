# Manual do Usuário: moonhttp (`http.moon`)

O **http.moon** (também chamado de `moonhttp`) é um servidor HTTP local superleve, rápido e visual, desenvolvido em Lua e projetado para rodar como um executável binário autônomo e multiplataforma (Linux, Windows e macOS). 
...
O **http.moon** implementa as características essenciais para um ambiente de desenvolvimento *localhost*:
...
Ao servir qualquer documento HTML ou `.lwa`, o **http.moon** analisa o código em tempo real e inspeciona as tags `<link>`, `<script>`, `<img>` e `<a href>`. Se o arquivo referenciado não existir na pasta local, o servidor emite um **alerta visual no terminal** informando a ausência do recurso.
...
* **Builds de Frameworks JS (React, Vue, Svelte, Angular):** O **http.moon** consegue servir com perfeição a pasta de exportação estática dessas ferramentas (pastas como `dist/` ou `build/`) após a execução do comando de *build* do framework.
...
* **v1.1.0:** Adição do **Inspetor de Dependências Estáticas**, exibição de logotipo em ASCII Art e links clicáveis no terminal com códigos de cores ANSI.
* **v1.2.0:** Adição do tratamento de rotas de pastas e diretórios sem barra final com redirecionamento HTTP `301`.
* **v1.3.0:** Adição de restrição de segurança para métodos HTTP (`GET`/`HEAD`), cabeçalhos contra cache de desenvolvimento e suporte à flag `--dir-list`.
...
### Parâmetros e ArgumentosFlag

```
Descrição   Valor Padrão
-h:<host>Define a interface de rede / host para escutar 
127.0.0.1

-p:<porta>Configura a porta de rede do servidor7777 (ou 8888 para perfis moon/moonip)

--dir-listAtiva a exibição da lista de arquivos da pasta caso não exista um arquivo indexDesativado (Retorna 404)Exemplos Práticos de Uso1. Execução Padrão (Mais Básico)Inicia o servidor no diretório atual (CWD), usando o IP 127.0.0.1 e a porta 7777.Bash./moonhttp
No Windows:DOSmoonhttp.exe
```
